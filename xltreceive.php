...
if(isset($_POST['submit'])){
	...
	$particulars = "From ".$_POST['despatchFrom']." To ".$row_loc['location_name'];
	/*-----------------------------*/
	if($_POST['submit']=="update"){
		if($_POST['location']!=$location_id){
			$sql = mysql_query("SELECT Max(xlt_no) as maxno FROM tblxlt WHERE location_id=".$_POST['location']." AND xlt_type='R' AND (xlt_date BETWEEN '".date("Y-m-d",strtotime($_SESSION['stores_syr']))."' AND '".date("Y-m-d",strtotime($_SESSION['stores_eyr']))."')");
			$row = mysql_fetch_assoc($sql);
			$xno = $row["maxno"] + 1;
		} else {
			$xno = $xlt_no;
		...
		$sql .= "',location_id=".$_POST['location'].",tfr_location='".$_POST['despatchFrom']."',tfr_by=".$_POST['staffName'].",despatch_mode=".$_POST['despatchMode'].",vehicle_num='".$_POST['vehicleNumber']."' WHERE xlt_id=".$xid;
		...
		$sql = "INSERT INTO logbook (rec_id,voucher_id,voucher_date,voucher_type,entry_date,particulars,location,action,user) VALUES(".$recordid.",'".$voucherid."','".$dateXLT."','XLT.Rcvd.','".date("Y-m-d")."','".$particulars."','".$_SESSION['stores_lname']."','Change','".$_SESSION['stores_uname']."')";
		$res = mysql_query($sql) or die(mysql_error());
		//end of inserting record into logbook
		echo '<script language="javascript">window.location="xltritem.php?action=new&xid='.$xid.'";</script>';
		...
		$recordid = ($row["maxid"]==null ? 1 : $row["maxid"] + 1);
		$voucherid = ($xid>999 ? $xid : ($xid>99 && $xid<1000 ? "0".$xid : ($xid>9 && $xid<100 ? "00".$xid : "000".$xid)));
		$sql = "INSERT INTO logbook (rec_id,voucher_id,voucher_date,voucher_type,entry_date,particulars,location,action,user) VALUES(".$recordid.",'".$voucherid."','".$dateXLT."','XLT.Rcvd.','".date("Y-m-d")."','".$particulars."','".$_SESSION['stores_lname']."','Delete','".$_SESSION['stores_uname']."')";
		$res = mysql_query($sql) or die(mysql_error());
		//end of inserting record into logbook
		echo '<script language="javascript">window.location="xltreceive.php?action=new";</script>';
		...
		$sql .= "'".$dateXLT."','R',".$_POST['location'].",'".$_POST['despatchFrom']."',".$_POST['staffName'].",".$_POST['despatchMode'].",'".$_POST['vehicleNumber']."')";
		
		$res = mysql_query($sql) or die(mysql_error());
		echo '<script language="javascript">function show_message_xlt_number(value1,value2){
			alert("XLT No. = "+value2);
			window.location="xltritem.php?action=new&xid="+value1;}
...
<head>
    ...
    function validate_xltdata() {
        ...
        if (document.getElementById("xltDate").value != "") {
            if (!checkdate(document.xltreceive.xltDate)) {
                return false;
            } else {
                var no_of_days1 = getDaysbetween2Dates(document.xltreceive.xltDate, document.xltreceive.endYear);
                if (no_of_days1 < 0) {
                    err += "* XLT date wrongly selected. Please correct and submit again.\n";
                } else {
                    var no_of_days2 = getDaysbetween2Dates(document.xltreceive.startYear, document.xltreceive.xltDate);
                    if (no_of_days2 < 0) {
                        err += "* XLT date wrongly selected. Please correct and submit again.\n";
                    } else {
                        var no_of_days3 = getDaysbetween2Dates(document.xltreceive.maxDate, document.xltreceive
                            .xltDate);
                        if (no_of_days3 < 0) {
                            err += "* XLT date wrongly selected. Please correct and submit again.\n" +
        ...
        if (document.getElementById("despatchFrom").value == "")
            err += "* please input source location, where material being transferred from!\n";
        if (document.getElementById("location").value == 0)
            err += "* please select receive location, where material being transferred to!\n";
        if (document.getElementById("staffName").value == 0)
            err += "* please select staff, who received the material!\n";
    ...
    function paging_xltr() {
        if (document.getElementById("xson").value == "new") {
            window.location = "xltreceive.php?action=" + document.getElementById("xson").value + "&pg=" + document
            ...
            window.location = "xltreceive.php?action=" + document.getElementById("xson").value + "&xid=" + document
    ...
    function firstpage_xltr() {
        document.getElementById("page").value = 1;
        paging_xltr();
    }

    function previouspage_xltr() {
...
<body>
        ...
        <table align="center" cellspacing="0" cellpadding="0" height="250px" width="875px" border="0">
            <tr>
                <td valign="top" colspan="3">
                    <form name="xltreceive" method="post" onsubmit="return validate_xltdata()">
                                ...
                                <td valign="top">
                                    <table class="Header" width="100%" cellspacing="0" cellpadding="0" border="0">
                                        <tr>
                                            ...
                                            <td class="th"><strong>External Location Transfer - [ Receive Main
                                                    ]</strong></td>
                                    ...
                                    <table class="Record" width="100%" cellspacing="0" cellpadding="0">
                                        ...
                                        <tr class="Controls">
                                            ...
                                            <td class="th">Receive At:<span style="color:#FF0000">*</span></td>
			...
			if($row_user['xlt1']==1){?>
                                                ...
                                                <input type="image" name="submit" src="images/update.gif" width="82"
                                                    ...
                                                    value="update" />&nbsp;&nbsp;<a
                                                    href="javascript:window.location='xltritem.php?action=new&xid=<?php echo $xid;?>'"><img
                                                        src="images/next.gif" width="72" height="22"
                                                        style="display:inline; cursor:hand;"
                                                        border="0" /></a>&nbsp;&nbsp;<a
                                                    href="javascript:window.location='xltreceive.php?action=new'"><img
                                                        src="images/reset.gif" width="72" height="22"
                                                        style="display:inline; cursor:hand;" border="0" /></a>
            ...
            <tr>
                <td valign="top" colspan="3">
                    <form name="xltlist" method="post" onsubmit="return validate_xltlist()">
                        <table align="center" width="1005" cellspacing="0" cellpadding="0" border="0">
                            <tr>
                                <td valign="top">
                                    <table class="Header" cellspacing="0" cellpadding="0" border="0" width="100%">
                                        <tr>
                                            <td class="HeaderLeft"><img src="images/spacer.gif" border="0" alt=""></td>
                                            <td class="th"><strong>External Location Transfer - [ Receive List
                                                    ]</strong></td>
                                            <td class="HeaderRight"><img src="images/spacer.gif" border="0" alt=""></td>
                                        </tr>
                                    </table>
	...
	implementation of the dynamically generated pages. -->
		...
		if($_SESSION['stores_utype']=="A" || $_SESSION['stores_utype']=="S")
			$sql_xlt = mysql_query("SELECT tblxlt.*, location_name, staff_name FROM tblxlt INNER JOIN location ON tblxlt.location_id = location.location_id INNER JOIN staff ON tblxlt.tfr_by = staff.staff_id WHERE xlt_type='R' AND xlt_date BETWEEN '".$fromDate."' AND '".$toDate."' ORDER BY location_name,xlt_date,xlt_id LIMIT ".$start.",".$end) or die(mysql_error());
		elseif($_SESSION['stores_utype']=="U")
		...
		while($row_xlt=mysql_fetch_array($sql_xlt)){
			...
			$i++;
			echo '<tr class="Row">';
			$edit_ref = "xltreceive.php?action=edit&xid=".$row_xlt['xlt_id'];
			$delete_ref = "xltreceive.php?action=delete&xid=".$row_xlt['xlt_id'];
			...
			echo '<td align="center">'.$i.'.</td><td onmouseover="myShow(\'<b><u>This is XLT number '.$xlt_number.'</u></b><br/>'.$stext.'\', this)" onmouseout="myHint.hide()">'.$xlt_number.'</td><td align="center">'.date("d-m-Y",strtotime($row_xlt['xlt_date'])).'</td><td>'.$row_xlt['tfr_location'].'</td><td>'.$row_xlt['location_name'].'</td><td>'.$row_xlt['staff_name'].'</td>';
			if($row_user['xlt2']==1)
			...
			echo '</tr>';
                                        ...
                                        <tr class="Footer">
                                            <td colspan="8" align="center">
                                                <?php 
			if($_SESSION['stores_utype']=="A" || $_SESSION['stores_utype']=="S")
				$sql_total = mysql_query("SELECT * FROM tblxlt WHERE xlt_type='R' AND xlt_date BETWEEN '".$fromDate."' AND '".$toDate."'") or die(mysql_error());
			elseif($_SESSION['stores_utype']=="U")
				$sql_total = mysql_query("SELECT * FROM tblxlt WHERE location_id=".$_SESSION['stores_locid']." AND xlt_type='R' AND xlt_date BETWEEN '".$fromDate."' AND '".$toDate."'") or die(mysql_error());
			$tot_row=mysql_num_rows($sql_total);
			$total_page=0;
			echo 'Total <span style="color:red">'.$tot_row.'</span> records&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
			echo '<input type="button" name="show" id="show" value="Show:" onclick="paging_xltr()" />&nbsp;&nbsp;<input name="displayTotalRows" id="displayTotalRows" value="'.$end.'" maxlength="2" size="2"/> rows&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="hidden" name="xson" id="xson" value="'.$_REQUEST['action'].'"/><input type="hidden" name="xlid" id="xlid" value="'.$xid.'"/>';
			if($tot_row>$end){
				echo "Page number: ";
				$total_page=ceil($tot_row/$end);
				echo '<select name="page" id="page" onchange="paging_xltr()" style="vertical-align:middle">';
			...
			if($total_page>1 && $pg>1)
				echo '<input type="button" name="fistPage" id="firstPage" value=" << " onclick="firstpage_xltr()" />&nbsp;&nbsp;<input type="button" name="prevPage" id="prevPage" value=" < " onclick="previouspage_xltr()" />&nbsp;&nbsp;';
			if($total_page>1 && $pg<$total_page)
				echo '<input type="button" name="nextPage" id="nextPage" value=" > " onclick="nextpage_xltr()" />&nbsp;&nbsp;<input type="button" name="lastPage" id="lastPage" value=" >> " onclick="lastpage_xltr()" />';
			?>
                                            </td>
                                        </tr>
                                    </table>
