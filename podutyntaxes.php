...
if(isset($_REQUEST["rid"])){
	...
	if($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete"){
		$sql2 = mysql_query("SELECT tblpo_dtm.* FROM tblpo_dtm  WHERE rec_id=".$rid);
		$row2 = mysql_fetch_assoc($sql2);
		$ssat = $row2['ssat'];
		$feed = $row2['feed'];
		$calc = $row2['calc'];
		$dtm_id = $row2["dtm_id"];
		$dtm_percent = $row2["dtm_percent"];
	}
}
/*-------------------------------*/
if(isset($_POST['submit'])){
	...
	if($_POST['submit']=="update"){
		$res = mysql_query("UPDATE tblpo_dtm SET ssat='".$_POST['SSAT']."',feed='".$_POST['Feeding']."',calc='".$_POST['Calc']."',dtm_id='".$_POST['dtmName']."',dtm_percent=".$dtmprcnt.",on_amount=".$_POST['onAmount'].",dtm_amount=".$dtmamt.",total_amount=".$_POST['totalAmount']." WHERE rec_id=".$rid) ;
		echo '<script language="javascript">window.location="podutyntaxes.php?action=calc&oid='.$oid.'";</script>';
	} elseif($_POST['submit']=="delete"){
		$res = mysql_query("DELETE FROM tblpo_dtm WHERE rec_id=".$rid) ;
		echo '<script language="javascript">window.location="podutyntaxes.php?action=calc&oid='.$oid.'";</script>';
	} elseif($_POST['submit']=="new"){
             $_POST['dtmName'];
		$sql = mysql_query("SELECT Max(rec_id) as maxid FROM tblpo_dtm");
		$row = mysql_fetch_assoc($sql);
		$rid = $row["maxid"] + 1;
		$sql = mysql_query("SELECT Max(seq_no) as maxid FROM tblpo_dtm WHERE po_id=".$oid);
		$row = mysql_fetch_assoc($sql);
		$sno = $row["maxid"] + 1;
		 $sql = "INSERT INTO tblpo_dtm (rec_id,po_id,seq_no,ssat,feed,calc,dtm_id,dtm_percent,on_amount,dtm_amount,total_amount) VALUES(".$rid.",".$oid.",".$sno.",'".$_POST['SSAT']."','".$_POST['Feeding']."','".$_POST['Calc']."','".$_POST['dtmName']."',".$dtmprcnt.",".$_POST['onAmount'].",".$dtmamt.",".$_POST['totalAmount'].")";
		$res = mysql_query($sql) ;
	}
}
?>
...
<tr>
	...
	<tr>
		...
		<tr>
			<td class="HeaderLeft"><img src="images/spacer.gif" border="0" alt=""></td>
			<td class="th"><strong>Purchase Order - [ Duties & Taxes ]</strong></td>
			<td class="HeaderRight"><img src="images/spacer.gif" border="0" alt=""></td>
		</tr>
		</table>
		...
		<table class="Grid" width="100%" cellspacing="0" cellpadding="0">
		<tr class="Caption">
			<th align="center" width="7%">SSATR</th>
			<th align="center" width="11%">Feeding</th>
			<th align="center" width="12%">Calc</th>
			<th align="center" width="25%">Description</th>
			<th align="center" width="10%">%age</th>
			<th align="center" width="12%">On Amount</th>
			<th align="center" width="10%">Amount</th>
			<th align="center" width="13%">Total Amt.</th>
		</tr>
		
		<tr class="Controls">
			<td><select name="SSAT" id="SSAT" style="width:50px"><?php 
			if($ssat=="Y"){
				echo '<option value="S">&nbsp;</option><option selected value="Y">Yes</option><option value="N">No</option>';
			} elseif($ssat=="N"){
				echo '<option value="S">&nbsp;</option><option value="Y">Yes</option><option selected value="N">No</option>';
			} else {
				echo '<option selected value="S">&nbsp;</option><option value="Y">Yes</option><option value="N">No</option>';
			}?>
			</select></td>
			
			<td><select name="Feeding" id="Feeding" style="width:75px" onchange="set_control_on_feeding(this.value)"><?php 
			if($feed=="A"){
				echo '<option value="S">&nbsp;</option><option selected value="A">Auto</option><option value="M">Manual</option>';
			} elseif($feed=="M"){
				echo '<option value="S">&nbsp;</option><option value="A">Auto</option><option selected value="M">Manual</option>';
			} else {
				echo '<option selected value="S">&nbsp;</option><option value="A">Auto</option><option value="M">Manual</option>';
			}?>
			</select></td>
			
			<td><select name="Calc" id="Calc" style="width:75px"><?php 
			if($calc=="P"){
				echo '<option value="S">&nbsp;</option><option selected value="P">Plus</option><option value="M">Minus</option>';
			} elseif($calc=="M"){
				echo '<option value="S">&nbsp;</option><option value="P">Plus</option><option selected value="M">Minus</option>';
			} else {
				echo '<option selected value="S">&nbsp;</option><option value="P">Plus</option><option value="M">Minus</option>';
			}?>
			</select></td>
			
			<td>
<!--                            <select name="dtmName" id="dtmName" style="width:200px"><option value="0">&nbsp;</option><?php 
//			$sqlDtm=mysql_query("SELECT * FROM dtm ORDER BY dtm_name");
//			while($rowDtm=mysql_fetch_array($sqlDtm)){
//				if($rowDtm["dtm_id"]==$dtm_id)
//					echo '<option selected value="'.$rowDtm["dtm_id"].'">'.$rowDtm["dtm_name"].'</option>';
//				else
//					echo '<option value="'.$rowDtm["dtm_id"].'">'.$rowDtm["dtm_name"].'</option>';
////			}?>
			</select>-->
                            <input type="text" name="dtmName" id="dtmName" style="width:200px" value="<?php echo $dtm_id ?>"/>
                        </td>
			
			<?php if($feed=="A"){
				echo '<td><span id="prcnt"><input name="Percentage" id="Percentage" maxlength="5" size="5" value="'.$dtm_percent.'" onchange="calculate_amount()"></span></td>';
			} elseif($feed=="M"){
				echo '<td><span id="prcnt"><input name="Percentage" id="Percentage" maxlength="5" size="5" readonly="true" value="'.$dtm_percent.'" onchange="calculate_amount()" style="background-color:#E7F0F8; color:#0000FF"></span></td>';
			} else {
				echo '<td><span id="prcnt"><input name="Percentage" id="Percentage" maxlength="5" size="5" value="" onchange="calculate_amount()"></span></td>';
			}?>
			
			<td><input name="onAmount" id="onAmount" maxlength="15" size="10" readonly="true" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){ echo $row2["on_amount"];} else { echo number_format($total_amount,2,".","");} ?>" style="background-color:#E7F0F8; color:#0000FF"></td>
			
			<td><span id="dtmamt"><input name="dtmAmount" id="dtmAmount" maxlength="15" size="10" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){ echo $row2["dtm_amount"];}?>" onchange="calculate_amount()"></span></td>
		
			<td><input name="totalAmount" id="totalAmount" maxlength="15" size="10" readonly="true" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){ echo $row2["on_amount"];} else { echo number_format($total_amount,2,".","");} ?>" style="background-color:#E7F0F8; color:#0000FF"></td>
		
		<?php if($msg!=""){echo '<tr class="Controls"><td colspan="8" align="center" style="color:#FF0000; font-weight:bold">'.$msg.'</td></tr>';} ?>
		
 		<tr class="Bottom">
			<td align="left" colspan="8">
		<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="new" || $_REQUEST["action"]=="calc")){
			if($row_user['po1']==1){?>
			...
			<?php }?>
&nbsp;&nbsp;<a href="javascript:document.podutyntaxes.reset()"><img src="images/reset.gif" width="72" height="22" style="display:inline; cursor:hand;" border="0"/></a>
		<?php } elseif(isset($_REQUEST["action"]) && $_REQUEST["action"]=="edit"){?>
			<input type="image" name="submit" src="images/update.gif" width="82" height="22" alt="update"><input type="hidden" name="submit" value="update"/>&nbsp;&nbsp;<a href="javascript:window.location='podutyntaxes.php?action=new&oid=<?php echo $oid;?>'" ><img src="images/reset.gif" width="72" height="22" style="display:inline; cursor:hand;" border="0" /></a>
		<?php } elseif(isset($_REQUEST["action"]) && $_REQUEST["action"]=="delete"){?>
			<input type="image" name="submit" src="images/delete.gif" width="72" height="22" alt="delete"><input type="hidden" name="submit" value="delete"/>&nbsp;&nbsp;<a href="javascript:window.location='podutyntaxes.php?action=new&oid=<?php echo $oid;?>'" ><img src="images/reset.gif" width="72" height="22" style="display:inline;cursor:hand;" border="0" /></a>
		<?php }
...
&nbsp;&nbsp;<a href="javascript:window.location='podutyntaxes.php?action=calc&oid=<?php echo $oid;?>'" ><img src="images/calculate.gif" width="93" height="22" style="display:inline;cursor:hand;" border="0" /></a>&nbsp;&nbsp;<a href="javascript:window.location='purchaseitem.php?action=new&oid=<?php echo $oid;?>&mul=y'" ><img src="images/back.gif" width="72" height="22" style="display:inline;cursor:hand;" border="0" /></a>&nbsp;&nbsp;<a href="javascript:window.location = 'pospec.php?action=new&oid=<?php echo $_REQUEST['oid'] ?>'" ><img src="images/next.gif" width="72" height="22" style="display:inline;cursor:hand;" border="0" /></a>&nbsp;&nbsp;
			</td>
		</tr>
		</table>
		</td>
...
<tr>
	...
	<tr>
		...
		<tr class="Caption">
			<th width="5%">Sl.No.</th>
			<th width="5%">SSATR</th>
			<th width="5%">Feeding</th>
			<th width="5%">Calc</th>
			<th width="30%">Description</th>
			<th width="5%">%age</th>
			<th width="12%">On Amount</th>
		...
		<?php 
		$i = 0;
		$sql_dtm = mysql_query("SELECT tblpo_dtm.* FROM tblpo_dtm   WHERE po_id=".$oid." ORDER BY seq_no") ;
		while($row_dtm=mysql_fetch_array($sql_dtm))
		{
			$i++;
			echo '<tr class="Row">';
			$delete_ref = "podutyntaxes.php?action=delete&oid=".$oid."&rid=".$row_dtm['rec_id'];
			$edit_ref = "podutyntaxes.php?action=edit&oid=".$oid."&rid=".$row_dtm['rec_id'];
			...
			echo '<td align="center">'.$i.'.</td><td align="center">'.$ssat.'</td><td align="center">'.$feed.'</td><td align="center">'.$calc.'</td><td>'.$row_dtm['dtm_id'].'</td><td align="center">'.$row_dtm['dtm_percent'].'%</td><td>'.$row_dtm['on_amount'].'</td><td>'.$row_dtm['dtm_amount'].'</td><td>'.$row_dtm['total_amount'].'</td>';
			if($row_user['po2']==1)
...
<tr>
	...
	<tr>
		...
		<tr class="Caption">
			...
			<th width="15%"><?php echo number_format($total_amount,2,".","");?></th>
		</tr>
		</table>
		</td>
	</tr>
