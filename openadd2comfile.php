	...
	if($_POST['submit']=="update"){
		if($count>0){
			...
			elseif($row_company['company_id']==$cid){
				$res = mysql_query("UPDATE company SET company_name='".$_POST['companyName']."',CCode='".$_POST['CCode']."',c_address1='".$_POST['address1']."',c_address2='".$_POST['address2']."',c_address3='".$_POST['address3']."',c_cityid=".$_POST['city'].",c_phone='".$_POST['phoneNo']."',c_fax='".$_POST['faxNo']."',c_email='".$_POST['emailID']."',c_tin='".$_POST['tin']."',c_cst='".$_POST['cst']."', c_pan='".$_POST['pan']."', c_gst='".$_POST['gst']."', c_cin='".$_POST['cin']."'  WHERE company_id=".$cid) or die(mysql_error());
				echo '<script language="javascript">window.location="openadd2comfile.php?action=new";</script>';
				//header("location:openadd2comfile.php?action=new");
			}
		} elseif($count==0){
			$res = mysql_query("UPDATE company SET company_name='".$_POST['companyName']."',CCode='".$_POST['CCode']."',c_address1='".$_POST['address1']."',c_address2='".$_POST['address2']."',c_address3='".$_POST['address3']."',c_cityid=".$_POST['city'].",c_phone='".$_POST['phoneNo']."',c_fax='".$_POST['faxNo']."',c_email='".$_POST['emailID']."',c_tin='".$_POST['tin']."',c_cst='".$_POST['cst']."', c_pan='".$_POST['pan']."', c_gst='".$_POST['gst']."', c_cin='".$_POST['cin']."' WHERE company_id=".$cid) or die(mysql_error());
			echo '<script language="javascript">window.location="openadd2comfile.php?action=new";</script>';
		...
		if($count>0)
			...
			$row = mysql_fetch_assoc($sql);
			$cid = $row["maxid"] + 1;
			$sql = "INSERT INTO company (company_id,company_name,CCode,c_address1,c_address2,c_address3,c_cityid,c_phone,c_fax,c_email,c_tin,c_cst,c_pan,c_gst,c_cin) VALUES(".$cid.",'".$_POST['companyName']."','".$_POST['CCode']."','".$_POST['address1']."','".$_POST['address2']."','".$_POST['address3']."',".$_POST['city'].",'".$_POST['phoneNo']."','".$_POST['faxNo']."','".$_POST['emailID']."','".$_POST['tin']."','".$_POST['cst']."','".$_POST['pan']."','".$_POST['gst']."','".$_POST['cin']."')";
...
function paging_cmp()
{
	if(document.getElementById("xson").value=="new")
		window.location="openadd2comfile.php?action="+document.getElementById("xson").value+"&pg="+document.getElementById("page").value+"&tr="+document.getElementById("displayTotalRows").value;
	else
		window.location="openadd2comfile.php?action="+document.getElementById("xson").value+"&cid="+document.getElementById("cmpid").value+"&pg="+document.getElementById("page").value+"&tr="+document.getElementById("displayTotalRows").value;
}
...
<tr>
	...
	<tr>
		...
		<tr class="Controls">
			...
			<td class="th" nowrap>Company Code:</td>
			<td><input name="CCode" id="CCode" maxlength="50" size="40" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){ echo $row["CCode"];}?>" tabindex="6"></td>
		</tr>
		...
		<tr class="Controls">
			...
			<td><input name="pan" id="pan" maxlength="15" size="20" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){ echo $row["c_pan"];}?>" tabindex="10"></td>
		    <td class="th" nowrap>G.S.T.:</td>
			<td><input name="gst" id="gst" maxlength="15" size="20" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){ echo $row["c_gst"];}?>" tabindex="10"></td>
		</tr>
		<tr class="Controls">
			<td class="th" nowrap>C.I.N.:</td>
			<td><input name="cin" id="cin" maxlength="15" size="20" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){ echo $row["c_cin"];}?>" tabindex="10"></td>
		    <td class="th" nowrap>T.I.N.:</td>
		  <td><input name="tin" id="tin" maxlength="15" size="20" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){ echo $row["c_tin"];}?>" tabindex="9"></td>
		</tr>
		<tr class="Controls">
		  <td class="th" nowrap>C.S.T.:</td>
		  <td><input name="cst" id="cst" maxlength="15" size="20" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){ echo $row["c_cst"];}?>" tabindex="10"></td>
		...
		<?php } elseif(isset($_REQUEST["action"]) && $_REQUEST["action"]=="edit"){?>
			<input type="image" name="submit" src="images/update.gif" width="82" height="22" alt="update" tabindex="11"><input type="hidden" name="submit" value="update"/>&nbsp;&nbsp;<a href="javascript:window.location='openadd2comfile.php?action=new'" tabindex="12"><img src="images/reset.gif" width="72" height="22" style="display:inline; cursor:hand;" border="0" /></a>
		<?php } elseif(isset($_REQUEST["action"]) && $_REQUEST["action"]=="delete"){?>
			<input type="image" name="submit" src="images/delete.gif" width="72" height="22" alt="delete" tabindex="11"><input type="hidden" name="submit" value="delete"/>&nbsp;&nbsp;<a href="javascript:window.location='openadd2comfile.php?action=new'" tabindex="12"><img src="images/reset.gif" width="72" height="22" style="display:inline; cursor:hand;" border="0" /></a>
		<?php }?>
...
<tr>
	...
	<tr>
		...
		while($row_company=mysql_fetch_array($sql_company))
		{
			$i++;
			echo '<tr class="Row">';
			$delete_ref = "openadd2comfile.php?action=delete&cid=".$row_company['company_id'];
			$edit_ref = "openadd2comfile.php?action=edit&cid=".$row_company['company_id'];
			
			echo '<td align="center">'.$i.'.</td><td>'.$row_company['company_name'].'</td>
			<td>'.$row_company['CCode'].'</td>
			<td>'.$row_company['city_name'].'</td><td>'.$row_company['state_name'].'</td>';
