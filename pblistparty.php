...
<head>
    ...
    function paging_pblist() {
        window.location = "pblistparty.php?pid=" + document.getElementById("party").value + "&pg=" + document
...
<body>
    ...
    <form name="pblist" id="pblist" method="post" action="pblistparty.php" onsubmit="return validate_dateselection()">
        <table align="center" border="0" cellpadding="2" cellspacing="1" width="875px">
            <tbody>
                <tr align="center"
                    ...
                    <td>Party wise Purchase Bill List</td>
                </tr>
                <tr align="center"
	...
	$sql = "SELECT tblbill.*, party_name, company_name FROM tblbill INNER JOIN party ON tblbill.party_id = party.party_id INNER JOIN company ON tblbill.company_id = company.company_id WHERE bill_return=0 AND (bill_date BETWEEN '".date("Y-m-d",$sm)."' AND '".date("Y-m-d",$em)."')";
	if($pid!=0){
		$sql .= " AND tblbill.party_id=".$pid;
	}
	$sql .= " ORDER BY party_name, bill_date, bill_no LIMIT ".$start.",".$end;
		...
		while($row_item=mysql_fetch_array($sql_item)){
			...
			echo '</tr>';
                ...
                <tr>
                    <td align="center">
                        <?php 
	$sql = "SELECT * FROM tblbill WHERE bill_return=0 AND (bill_date BETWEEN '".date("Y-m-d",$sm)."' AND '".date("Y-m-d",$em)."')";
	if($pid!=0){
		$sql .= " AND tblbill.party_id=".$pid;
	}
