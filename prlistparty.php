...
<head>
    ...
    function paging_prlist() {
        window.location = "prlistparty.php?pid=" + document.getElementById("party").value + "&pg=" + document
...
<body>
    ...
    <form name="prlist" id="prlist" method="post" action="prlistparty.php" onsubmit="return validate_dateselection()">
        <table align="center" border="0" cellpadding="2" cellspacing="1" width="875px">
            <tbody>
                <tr align="center"
                    ...
                    <td>Party wise Purchase Return List</td>
                </tr>
                <tr align="center"
	...
	$sql = "SELECT tblbill.*, party_name, company_name FROM tblbill INNER JOIN party ON tblbill.party_id = party.party_id INNER JOIN company ON tblbill.company_id = company.company_id WHERE bill_return=1 AND (bill_date BETWEEN '".date("Y-m-d",$sm)."' AND '".date("Y-m-d",$em)."')";
	if($pid!=0){
	...
	while($row=mysql_fetch_array($res)){
		...
		$sql_item = mysql_query("SELECT tblbill_item.*, po_no, receipt_no, receipt_prefix, item_name, unit_name,ic.category FROM tblbill_item INNER JOIN tblpo ON tblbill_item.po_id = tblpo.po_id INNER JOIN tblreceipt1 ON tblbill_item.receipt_id = tblreceipt1.receipt_id INNER JOIN item_category ic ON ic.category_id = tblbill_item.item_category INNER JOIN item ON tblbill_item.item_id = item.item_id INNER JOIN unit ON tblbill_item.unit_id = unit.unit_id WHERE bill_id=".$row['bill_id']." ORDER BY seq_no") or die(mysql_error());
		while($row_item=mysql_fetch_array($sql_item)){
			...
			echo '</tr>';
                ...
                <tr>
                    <td align="center">
                        <?php 
	$sql = "SELECT * FROM tblbill WHERE bill_return=1 AND (bill_date BETWEEN '".date("Y-m-d",$sm)."' AND '".date("Y-m-d",$em)."')";
	if($pid!=0){
		$sql .= " AND tblbill.party_id=".$pid;
	}
