	...
	if($_POST['submit']=="update"){
		if($count>0){
			...
			elseif($row_city['city_id']==$cid){
				...
				echo '<script language="javascript">window.location="city.php?action=new";</script>';
			}
		} elseif($count==0){
			$res = mysql_query("UPDATE city SET city_name='".$_POST['cityName']."',state_id=".$_POST['state']." WHERE city_id=".$cid) or die(mysql_error());
			echo '<script language="javascript">window.location="city.php?action=new";</script>';
...
function paging_city(value1)
{
	if(document.getElementById("xson").value=="new")
		window.location="city.php?action="+document.getElementById("xson").value+"&pg="+document.getElementById("page").value+"&tr="+document.getElementById("displayTotalRows").value+value1;
	else
		window.location="city.php?action="+document.getElementById("xson").value+"&cid="+document.getElementById("ctyid").value+"&pg="+document.getElementById("page").value+"&tr="+document.getElementById("displayTotalRows").value+value1;
}
...
<tr>
	...
	<tr>
		...
		<?php }?>
&nbsp;&nbsp;<a href="javascript:window.location='city.php?action=new'"><img src="images/reset.gif" width="72" height="22" style="display:inline; cursor:hand;" border="0" /></a>
...
<tr>
	...
	<tr>
		...
		while($row_city=mysql_fetch_array($sql_city))
		{
			$i++;
			echo '<tr class="Row">';
			$delete_ref = "city.php?action=delete&cid=".$row_city['city_id'];
			$edit_ref = "city.php?action=edit&cid=".$row_city['city_id'];
