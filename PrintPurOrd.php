...
<head>
<script>
function FunPrintPage(id)
{ 
 window.print();
 window.close();
}
</script>
<style>section{ font-size:12px;	} table{ font-size:12px; }
	</style>
</head>
<body onLoad="FunPrintPage(<?php echo $_REQUEST['po_id']; ?>)">
<?php /*
	<input type="button" onClick="funPrint()" value="print">
	<input type="button" onClick="window.close()" value="Close">
*/ ?>	
<?php
$datapo=mysql_query("select * from tblpo where po_id=".$_REQUEST['po_id']);
  ...
  <td width="55%" valign="top">
   <table border="0">
    <tr>
	 ...
	 <td style="width:5px;"></td>
	 <td>
      <font style="font-size:18px;font-family:Times New Roman;"><b><?php echo $recCompany['company_name'] ?></b></font><br>
	  ...
	  <font style="font-size:14px;font-family:Times New Roman;"><b>VENDOR :&nbsp;<?php echo $recParty['party_name']; ?></b></font><br>
	  <font style="font-size:14px;font-family:Times New Roman;">
...
?>
   ...
   <table border="0" style="font-size:14px;font-family:Times New Roman;">
	 ...
	 <tr><td colspan="3"><font style="font-size:24px;"><b><?php echo $Tit; ?></b></font></td></tr><!--"000".$recpo['po_no']."/-->
	 <tr><td style="width:100px;"><b><?php echo $Tn; ?>.O. NO</b></td><td>:</td><td><b><?php echo $recCompany['CCode'].'/'.$pon."/".$yer; ?></b></td></tr>
	 <tr><td style="width:100px;"><b>DATE</b></td><td>:</td><td><b><?php echo date('d-m-Y',strtotime($recpo['po_date'])) ?></b></td></tr>
	 <tr><td colspan="3">&nbsp;</td></tr>
	 <tr><td style="width:100px;">VENDOR ID</td><td>:</td><td><?php echo $recpo['party_id']; ?></td></tr>
	 <tr><td style="width:100px;">VENDOR REF</td><td>:</td><td><?php echo ''; ?></td></tr>
     <tr><td style="width:100px;">Vendor's GST</td><td>:</td><td><?php echo strtoupper($recParty['gstno']); ?></td></tr>
	 <tr><td style="width:100px;">Vendor's PAN</td><td>:</td><td><?php echo strtoupper($recParty['pan']); ?></td></tr>
	 <tr><td style="width:100px;">No</td><td>:</td><td></td></tr>
...
<?php $data2City=mysql_query("select city_name from city where city_id=".$rec2Company['c_cityid']); $rec2City=mysql_fetch_array($data2City); ?> 
	 <tr><td style="width:100px;" valign="top"><b>SHIP TO</b></td><td valign="top">:</td><td valign="top"><b><?php echo $rec2Company['company_name']; ?></b></td></tr>
	 <tr><td style="width:100px;" colspan="2"></td><td><?php echo $rec2Company['c_address1'].', '.$rec2Company['c_address2']; ?></td></tr>
	 <tr><td style="width:100px;" colspan="2"></td><td><?php echo $rec2Company['c_address3']; ?></td></tr>
	 <tr><td style="width:100px;" colspan="2"></td><td><?php echo strtoupper($rec2City['city_name']); ?></td></tr>
	 <tr><td style="width:100px;" colspan="2"></td><td>Phone No-<?php echo $rec2Company['c_phone']; ?></td></tr>
   </table>
  </td>
 </tr>
  ...
  <td colspan='2'>
   <table width='100%' border="1px" style="border-collapse:collapse;text-align:center;font-size:11px;">
	...
	<tr style="background-color:#FFFFFF;font-size:12px;">
	 <td colspan="2"><?php if($recpo["ship_method"]) echo $recpo['ship_method']; else echo"Nil"?></td>
	  <td colspan="2"><?php if($recpo['ship_terms']) echo $recpo['ship_terms']; else echo "Nil"?></td>
	  <td colspan="2"><b><?php if($recpo['delivery_date']) echo date("d-m-Y",strtotime($recpo['delivery_date'])); else echo "Immediately"; ?></b></td>
    </tr>
	<tr style="background-color:#DCDCBA;">
	 <th style="width:40px;">QNo</th>
	 <th style="width:120px;">ITEM NAME</th>
         <?php if($rrow>0){ ?><th style="width:200px;">Remark</th><?php } ?>
	 <th style="width:300px;">DESCRIPTION</th>
	 <th style="width:60px;">QTY</th>
	 <th style="width:60px;">UNIT PRICE</th>
	 <th style="width:80px;">LINE TOTAL</th>
	</tr>			
<?php $i=0; $dataItem=mysql_query("select * from tblpo_item where po_id=".$recpo['po_id']); $sum=0; while($recItem=  mysql_fetch_array($dataItem)){
 echo "<tr>";
  echo "<td align='center'>".++$i."</td>";
$dataItemName=mysql_query("select item_name from item where item_id=".$recItem['item_id']); $recItemName=  mysql_fetch_array($dataItemName);
  echo "<td align='left'>".$recItemName['item_name']."</td>";

  if($rrow>0){ $sqOth=mysql_query("select AnyOther from tbl_indent_item where indent_id=".$rchk['indent_id']." AND item_id=".$recItem['item_id']); $rqOth=mysql_fetch_assoc($sqOth);  echo "<td align='left'>".$rqOth['AnyOther']."</td>"; }  

  echo "<td align='left'>".$recItem['item_description']."</td>";
  echo "<td align='center'>".floatval($recItem['qnty'])."</td>";
...
?>
   <tr>
	<td colspan="<?php if($rrow>0){echo 4;}else{echo 3;}?>" rowspan="10" style="border-left:hidden;">
...
<?php $spec=mysql_query("select * from tblpo_spec where po_id=".$_REQUEST["po_id"]); while($rec=mysql_fetch_array($spec)){
 echo '<tr><td align="center" style="width:2px;">*</td><td align="left">&nbsp;'.$rec["specification"].'</td></tr>'; } ?>
	...
	<td colspan="2" align="right" style="font-size:11px;background-color:#DCDCBA;"><b>SUM AMOUNT&nbsp;<b></td>
...
<?php $dataDtm=mysql_query("select * from tblpo_dtm where po_id=".$recpo['po_id']);  $rown=mysql_num_rows($dataDtm);
  ...
  <tr>
	<td colspan="2" align="right" style="font-size:11px;background-color:#DCDCBA;"><b><?php echo $recDtm['dtm_id']; if($recDtm['dtm_percent']>0){ echo  ' ('.$recDtm['dtm_percent'].'%)'; }?>&nbsp;<b></td>
	<td align="right" style="font-size:12px;"><?php if($recDtm['calc']=='M') echo "- ".number_format($recDtm['dtm_amount'],2); else echo number_format($recDtm['dtm_amount'],2); ?>&nbsp;</td>  
  </tr>
  <?php if($rown==$i){ ?>
   <tr>
	<td colspan="2" align="right" style="font-size:11px;background-color:#DCDCBA;"><b>TOTAL NET AMOUNT&nbsp;<b></td>
	<td align="right" style="font-size:12px;"><b><?php echo number_format($recDtm['total_amount'],2); ?></b>&nbsp;</td>  
  </tr> 
<?php } $i++; } } ?>
 </table>
 </td>
