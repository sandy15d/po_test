...
if(isset($_POST['rid']) && $_POST['rid']!=""){
	...
	<tr class="Controls">
		...
		<td width="45%" id="tblItemList"><select name="item" id="item" disabled onchange="get_curent_stock_of_item(this.value,<?php echo $row1['order_from'];?>,'<?php echo strtotime($_SESSION['stores_syear']);?>','<?php echo strtotime($row1["indent_date"]);?>')" style="width:300px"><option value="0">-- Select --</option>
		<?php 
	...
	<tr class="Controls">
		...
		<td><input name="itemQnty" id="itemQnty" maxlength="10" size="10" readonly="true" value="<?php echo $row2["qnty"];?>" style="background-color:#E7F0F8; color:#0000FF">&nbsp;<span id="spanUnit2"><?php if($row2['alt_unit']=="N"){echo $row2["unit_name"];} elseif($row2['alt_unit']=="A"){echo "&nbsp;";}?></span></td>
		...
		<td id="tblcol2"><?php if($row2['alt_unit']=="N"){echo '<input type="hidden" name="unit" id="unit" value="0"/>';}
			elseif($row2['alt_unit']=="A" && $row2['alt_unit_id']!=0){
				echo '<select name="unit" id="unit" disabled style="width:115px"><option value="'.$row2['prime_unit_id'].'">'.$prime_unit_name.'</option><option value="'.$row2['alt_unit_id'].'">'.$alt_unit_name.'</option></select>';}?>
		</td>
	</tr>
	
	<tr class="Controls">
		<td class="th" nowrap>Remark:</td>
		<td colspan="3"><input name="remark" id="remark" maxlength="100" size="85" readonly="true" value="<?php echo ($row2["remark"]==null?"":$row2["remark"]); ?>" style="background-color:#E7F0F8; color:#0000FF"></td>
	</tr>
	
 	<tr class="Bottom">
		<td align="left" colspan="4">
			<span id="spanButton"><img id="submit" src="images/delete.gif" width="72" height="22" style="cursor:hand;" onclick="return validate_indent()"/></span>
