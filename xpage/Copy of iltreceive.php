...
include("menu.php");
/*--------------------------------*/
$sql_user = mysql_db_query(DATABASE2,"SELECT ilt1,ilt2,ilt3,ilt4 FROM users WHERE uid=".$_SESSION['stores_uid']) or die(mysql_error());
...
if(isset($_REQUEST["action"])){
	if($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete"){
		$sql = mysql_db_query(DATABASE2,"SELECT tblilt1.*, source.location_name AS sourceLocation, destination.location_name AS destinationLocation, staff_name FROM tblilt1 INNER JOIN ".DATABASE1.".location AS source ON tblilt1.despatch_from = source.location_id INNER JOIN ".DATABASE1.".location AS destination ON tblilt1.receive_at = destination.location_id INNER JOIN ".DATABASE1.".staff ON tblilt1.despatch_by = staff.staff_id WHERE ilt_id=".$mid) or die(mysql_error());
		$row = mysql_fetch_assoc($sql);
		$ino = $row['ilt_no'];
	} elseif($_REQUEST["action"]=="recall"){
		$sql = mysql_db_query(DATABASE2,"SELECT tblilt1.*, source.location_name AS sourceLocation, destination.location_name AS destinationLocation FROM tblilt1 INNER JOIN ".DATABASE1.".location AS source ON tblilt1.despatch_from = source.location_id INNER JOIN ".DATABASE1.".location AS destination ON tblilt1.receive_at = destination.location_id WHERE ilt_id=".$mid) or die(mysql_error());
		$row = mysql_fetch_assoc($sql);
		$dateILT = $row['receive_date'];
		$particulars = "From ".$row['sourceLocation']." To ".$row['destinationLocation'];
		$res = mysql_db_query(DATABASE2,"UPDATE tblilt1 SET receive_status='U' WHERE ilt_id=".$mid) or die(mysql_error());
		//insert into logbook
...
if(isset($_POST['submit'])){
	...
	if($_POST['submit']=="update"){
		$res = mysql_db_query(DATABASE2,"UPDATE tblilt1 SET receive_date='".$dateILT."', receive_by=".$_POST['receiveBy']." WHERE ilt_id=".$mid) or die(mysql_error());
		$res = mysql_db_query(DATABASE2,"UPDATE stock_register SET entry_date='".$dateILT."',location_id=".$row['receive_at'].",uid=".$_SESSION['stores_uid']." WHERE entry_mode='T+' AND entry_id=".$mid) or die(mysql_error());
		//insert into logbook
		$sql = mysql_db_query(DATABASE3,"SELECT Max(rec_id) as maxid FROM logbook");
		$row = mysql_fetch_assoc($sql);
		$recordid = ($row["maxid"]==null ? 1 : $row["maxid"] + 1);
		...
		$res = mysql_db_query(DATABASE2,"UPDATE tblilt1 SET receive_by=0, receive_status='U' WHERE ilt_id=".$mid) or die(mysql_error());
		$res = mysql_db_query(DATABASE2,"UPDATE tblilt2 SET receive_qnty=0 WHERE ilt_id=".$mid) or die(mysql_error());
		$res = mysql_db_query(DATABASE2,"DELETE FROM stock_register WHERE entry_mode='T+' AND entry_id=".$mid) or die(mysql_error());
		//insert into logbook
		$sql = mysql_db_query(DATABASE3,"SELECT Max(rec_id) as maxid FROM logbook");
		$row = mysql_fetch_assoc($sql);
		$recordid = ($row["maxid"]==null ? 1 : $row["maxid"] + 1);
		...
		echo '<script language="javascript">function show_message_ilt_number(value1){
			...
			show_message_ilt_number('.$ino.');</script>';
	}
}
?>
...
function validate_iltdata()
{
	...
	if(document.getElementById("receiveDate").value!=""){
		if(!checkdate(document.ilt.receiveDate)){
			return false;
		} else {
			var no_of_days = getDaysbetween2Dates(document.ilt.despatchDate,document.ilt.receiveDate);
			if(no_of_days < 0){
	...
	if(document.getElementById("receiveBy").value==0)
		err += "* please select a leader, by whom the material being received!\n";
	if(err=="")
		return true;
	else {
		alert("Error: \n"+err);
...
function paging_iltr()
{
	if(document.getElementById("xson").value=="new"){
		window.location="iltreceive.php?action="+document.getElementById("xson").value+"&pg="+document.getElementById("page").value+"&tr="+document.getElementById("displayTotalRows").value+"&rf="+document.getElementById("rf").value+"&sd="+document.getElementById("sd").value+"&ed="+document.getElementById("ed").value;
	} else {
		window.location="iltreceive.php?action="+document.getElementById("xson").value+"&mid="+document.getElementById("miid").value+"&pg="+document.getElementById("page").value+"&tr="+document.getElementById("displayTotalRows").value+"&rf="+document.getElementById("rf").value+"&sd="+document.getElementById("sd").value+"&ed="+document.getElementById("ed").value;
	}
}
...
<tr>
	...
	<form name="ilt"  method="post" onsubmit="return validate_iltdata()">
	...
	<tr>
		...
		<tr class="Controls">
			...
			<td><input name="iltNo" id="iltNo" maxlength="15" size="20" readonly="true" value="<?php echo $ilt_number; ?>" style="background-color:#E7F0F8; color:#0000FF"></td>
		...
		<tr class="Controls">
			...
			<td><input name="despatchDate" id="despatchDate" maxlength="10" size="10" readonly="true" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){echo date("d-m-Y",strtotime($row["ilt_date"]));}?>" style="background-color:#E7F0F8; color:#0000FF"></td>
			
			<td class="th">Receive Date:<span style="color:#FF0000">*</span></td>
			<td><input name="receiveDate" id="receiveDate" maxlength="10" size="10" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){echo date("d-m-Y",strtotime($row["receive_date"]));} else echo date("d-m-Y");?>" tabindex="1">&nbsp;<script language="JavaScript">new tcal ({'formname': 'ilt', 'controlname': 'receiveDate'});</script></td>
		</tr>
		
		<tr class="Controls">
			<td class="th">Despatch From:</td>
			<td><input name="despatchFrom" id="despatchFrom" maxlength="50" size="45" readonly="true" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){ echo $row['sourceLocation'];}?>" style="background-color:#E7F0F8; color:#0000FF"></td>
			
			<td class="th">Destination:</td>
			<td><input name="destination" id="destination" maxlength="50" size="45" readonly="true" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){ echo $row['destinationLocation'];}?>" style="background-color:#E7F0F8; color:#0000FF"></td>
		</tr>
		
		<tr class="Controls">
			<td class="th" nowrap>Despatched By:</td>
			<td><input name="despatchBy" id="despatchBy" maxlength="50" size="45" readonly="true" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){ echo $row['staff_name'];}?>" style="background-color:#E7F0F8; color:#0000FF"></td>
			
			<td class="th" nowrap>Received By:<span style="color:#FF0000">*</span></td>
			<td><select name="receiveBy" id="receiveBy" style="width:300px" tabindex="2">
			<option selected value="0">-- Select --</option>
			<?php 
			$sql_staff=mysql_db_query(DATABASE1,"SELECT * FROM staff WHERE location_id=".$row['receive_at']." ORDER BY staff_name");
			while($row_staff=mysql_fetch_array($sql_staff)){
				if($row_staff["staff_id"]==$row["receive_by"])
		...
		<tr class="Controls">
			...
			<td><input name="despatchMode" id="despatchMode" maxlength="50" size="20" readonly="true" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){ echo ($row['despatch_mode']==1 ? "Hand Delivery" : "By Vehicle");}?>" style="background-color:#E7F0F8; color:#0000FF"></td>
			
			<td class="th" nowrap>Vehicle No.:</td>
			<td><input name="vehicleNumber" id="vehicleNumber" maxlength="15" size="20" readonly="true" value="<?php if(isset($_REQUEST["action"]) && ($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete")){ echo $row['vehicle_num'];}?>" style="background-color:#E7F0F8; color:#0000FF"></td>
		</tr>
		...
		<?php if(isset($_REQUEST["action"]) && $_REQUEST["action"]=="edit"){?>
			<input type="image" name="submit" src="images/update.gif" width="82" height="22" alt="update" tabindex="3"><input type="hidden" name="submit" value="update"/>&nbsp;&nbsp;<a href="javascript:window.location='iltritem.php?action=new&mid=<?php echo $mid;?>'"><img src="images/next.gif" width="72" height="22" style="display:inline; cursor:hand;" border="0" /></a>&nbsp;&nbsp;<a href="javascript:window.location='iltreceive.php'" tabindex="4"><img src="images/reset.gif" width="72" height="22" style="display:inline; cursor:hand;" border="0" /></a>
		<?php } elseif(isset($_REQUEST["action"]) && $_REQUEST["action"]=="delete"){?>
			<input type="image" name="submit" src="images/delete.gif" width="72" height="22" alt="delete" tabindex="3"><input type="hidden" name="submit" value="delete"/>&nbsp;&nbsp;<a href="javascript:window.location='iltreceive.php'" tabindex="4"><img src="images/reset.gif" width="72" height="22" style="display:inline;cursor:hand;" border="0" /></a>
		<?php }?>
&nbsp;&nbsp;<a href="javascript:window.location='menu.php'" tabindex="5"><img src="images/back.gif" width="72" height="22" style="display:inline; cursor:hand;" border="0"/></a>
			</td>
		</tr>
		</table>
		</td>
...
<tr>
	...
	<tr>
		...
		<tr>
			<td class="HeaderLeft"><img src="images/spacer.gif" border="0" alt=""></td>
			<td class="th"><strong>Inter Location Transfer - [ Receive List ]</strong></td>
			<td class="HeaderRight"><img src="images/spacer.gif" border="0" alt=""></td>
		</tr>
		</table>
...
<!-- End of the HTML code for the hint -->
		...
		<tr class="Caption">
			<th align="right" colspan="9">List Range:&nbsp;&nbsp;<select name="rangeFor" id="rangeFor" style="width:140px" onchange="listrange(this.value)" tabindex="6">
			<?php 
			...
			</select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;From:&nbsp;&nbsp;<input name="rangeFrom" id="rangeFrom" maxlength="10" size="10" value="<?php echo date("d-m-Y",$sd);?>" tabindex="7">&nbsp;<script language="JavaScript">new tcal ({'formname': 'iltlist', 'controlname': 'rangeFrom'});</script>&nbsp;&nbsp;Range To:&nbsp;&nbsp;<input name="rangeTo" id="rangeTo" maxlength="10" size="10" value="<?php echo date("d-m-Y",$ed);?>" tabindex="8">&nbsp;<script language="JavaScript">new tcal ({'formname': 'iltlist', 'controlname': 'rangeTo'});</script>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="image" name="search" src="images/search.gif" width="82" height="22" alt="search" tabindex="9"><input type="hidden" name="sd" id="sd" value="<?php echo $sd;?>" /><input type="hidden" name="ed" id="ed" value="<?php echo $ed;?>" /></th>
		</tr>
		<tr class="Caption">
			<th width="5%">Sl.No.</th>
		...
		if($_SESSION['stores_utype']=="A" || $_SESSION['stores_utype']=="S")
			$sql_ilt = mysql_db_query(DATABASE2,"SELECT tblilt1.*, source.location_name AS sourceLocation, destination.location_name AS destinationLocation, despatchStaff.staff_name AS sourceStaff FROM tblilt1 INNER JOIN ".DATABASE1.".location AS source ON tblilt1.despatch_from = source.location_id INNER JOIN ".DATABASE1.".location AS destination ON tblilt1.receive_at = destination.location_id INNER JOIN ".DATABASE1.".staff AS despatchStaff ON tblilt1.despatch_by = despatchStaff.staff_id WHERE despatch_status='S' AND receive_status='".$rangeFor."' AND ilt_date BETWEEN '".$fromDate."' AND '".$toDate."' ORDER BY destinationLocation,ilt_date,ilt_id LIMIT ".$start.",".$end) or die(mysql_error());
		elseif($_SESSION['stores_utype']=="U")
			$sql_ilt = mysql_db_query(DATABASE2,"SELECT tblilt1.*, source.location_name AS sourceLocation, destination.location_name AS destinationLocation, despatchStaff.staff_name AS sourceStaff FROM tblilt1 INNER JOIN ".DATABASE1.".location AS source ON tblilt1.despatch_from = source.location_id INNER JOIN ".DATABASE1.".location AS destination ON tblilt1.receive_at = destination.location_id INNER JOIN ".DATABASE1.".staff AS despatchStaff ON tblilt1.despatch_by = despatchStaff.staff_id WHERE receive_at=".$_SESSION['stores_locid']." AND despatch_status='S' AND receive_status='".$rangeFor."' AND ilt_date BETWEEN '".$fromDate."' AND '".$toDate."' ORDER BY destinationLocation,ilt_date,ilt_id LIMIT ".$start.",".$end) or die(mysql_error());
		
		while($row_ilt=mysql_fetch_array($sql_ilt)){
			$sql_item = mysql_db_query(DATABASE2,"SELECT tblilt2.*, item_name, unit_name FROM tblilt2 INNER JOIN ".DATABASE1.".item ON tblilt2.item_id = item.item_id INNER JOIN ".DATABASE1.".unit ON tblilt2.unit_id = unit.unit_id WHERE ilt_id=".$row_ilt['ilt_id']." ORDER BY seq_no") or die(mysql_error());
			$j = 0;
		...
		<tr class="Footer">
			...
			<?php 
			if($_SESSION['stores_utype']=="A" || $_SESSION['stores_utype']=="S")
				$sql_total = mysql_db_query(DATABASE2,"SELECT * FROM tblilt1 WHERE despatch_status='S' AND receive_status='".$rangeFor."' AND ilt_date BETWEEN '".$fromDate."' AND '".$toDate."'") or die(mysql_error());
			elseif($_SESSION['stores_utype']=="U")
				$sql_total = mysql_db_query(DATABASE2,"SELECT * FROM tblilt1 WHERE receive_at=".$_SESSION['stores_locid']." AND despatch_status='S' AND receive_status='".$rangeFor."' AND ilt_date BETWEEN '".$fromDate."' AND '".$toDate."'") or die(mysql_error());
			$tot_row=mysql_num_rows($sql_total);
			$total_page=0;
			echo 'Total <span style="color:red">'.$tot_row.'</span> records&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
			echo '<input type="button" name="show" id="show" value="Show:" onclick="paging_iltr()" />&nbsp;&nbsp;<input name="displayTotalRows" id="displayTotalRows" value="'.$end.'" maxlength="2" size="2" tabindex="10" /> rows&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="hidden" name="xson" id="xson" value="'.$_REQUEST["action"].'" /><input type="hidden" name="miid" id="miid" value="'.$mid.'" />';
			if($tot_row>$end){
				echo "Page number: ";
