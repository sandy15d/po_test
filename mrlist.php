...
<head>
    ...
    function paging_mrlist() {
        window.location = "mrlist.php?lid=" + document.getElementById("location").value + "&pg=" + document
...
<body>
    ...
    <form name="mrlist" id="mrlist" method="post" action="mrlist.php" onsubmit="return validate_dateselection()">
                            ...
                            <table align="center" border="1" bordercolorlight="#7ECD7A" cellpadding="2" id="printTable"
                                ...
                                <tbody>
                                    <tr bgcolor="#E6E1B0" align="center"
                                        ...
                                        <td width="25%" valign="middle" style="border-top:none; border-left:none">Party
                                            Name
                                        </td>
	...
	$sql .= " ORDER BY receivedat, receipt_date, receipt_id LIMIT ".$start.",".$end;
	...
	while($row=mysql_fetch_array($res)){
		...
		if($row['recd_at']!=$recdAt){
			...
			echo '<td style="border-left:none; border-bottom:none; border-right:none; font-weight:bold" colspan="7">'.$row['receivedat'].'</td>';
			echo '</tr>';
		}
		...
		$sql_item = mysql_query("SELECT tblreceipt2.*, item_name, ic.category,unit_name FROM tblreceipt2 INNER JOIN item ON tblreceipt2.item_id = item.item_id 
        INNER JOIN item_category ic ON ic.category_id = tblreceipt2.item_category
        INNER JOIN unit ON tblreceipt2.unit_id = unit.unit_id WHERE receipt_id=".$row['receipt_id']." ORDER BY seq_no") or die(mysql_error());
		while($row_item=mysql_fetch_array($sql_item)){
			...
			echo '</tr>';
                ...
                <tr>
                    <td align="center">
                        <?php 
	$sql = "SELECT * FROM tblreceipt1 WHERE (receipt_date BETWEEN '".date("Y-m-d",$sm)."' AND '".date("Y-m-d",$em)."')";
	if($lid!=0){
		$sql .= " AND recd_at=".$lid;
	}
