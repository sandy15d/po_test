...
if(isset($_REQUEST['rid'])){
	...
	if($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete"){
		$sql2 = mysql_query("SELECT tblxlt_item.*, item_name, unit_name FROM tblxlt_item INNER JOIN item ON tblxlt_item.item_id = item.item_id INNER JOIN unit ON tblxlt_item.unit_id = unit.unit_id WHERE rec_id=".$rid) or die(mysql_error());
...
if(isset($_POST['submit'])){
	...
	$particulars = "From ".$row1['tfr_location']." To ".$row1['location_name'];
	/*----------------------------------------*/
	...
	if($_POST['submit']=="update"){
		if($count>0){
			...
			elseif($row_xlt['rec_id']==$rid){
				...
				$res=mysql_query("UPDATE stock_register SET item_id=".$_POST['item'].",unit_id=".$unitid.",item_qnty=".$itemQnty." WHERE entry_mode='X+' AND entry_id=".$xid." AND entry_date='".$dateXLT."' AND seq_no=".$row2['seq_no']." AND location_id=".$row1['location_id']) or die(mysql_error());
				//insert into logbook
				$sql = mysql_query("SELECT Max(rec_id) as maxid FROM logbook");
				$row = mysql_fetch_assoc($sql);
				$recordid = ($row["maxid"]==null ? 1 : $row["maxid"] + 1);
				$sql = "INSERT INTO logbook (rec_id,voucher_id,voucher_date,voucher_type,entry_date,particulars,item_name,unit,item_qnty,location,action,user) VALUES(".$recordid.",'".$voucherid."','".$dateXLT."','XLT.Rcvd.','".date("Y-m-d")."','".$particulars."','".$itemname."','".$unitname."',".$_POST['itemQnty'].",'".$_SESSION['stores_lname']."','Change','".$_SESSION['stores_uname']."')";
		...
		$row = mysql_fetch_assoc($sql);
		$recordid = ($row["maxid"]==null ? 1 : $row["maxid"] + 1);
		$sql = "INSERT INTO logbook (rec_id,voucher_id,voucher_date,voucher_type,entry_date,particulars,item_name,unit,item_qnty,location,action,user) VALUES(".$recordid.",'".$voucherid."','".$dateXLT."','XLT.Rcvd.','".date("Y-m-d")."','".$particulars."','".$itemname."','".$unitname."',".$_POST['itemQnty'].",'".$_SESSION['stores_lname']."','Delete','".$_SESSION['stores_uname']."')";
		$res = mysql_query($sql) or die(mysql_error());
		//end of inserting record into logbook
		echo '<script language="javascript">window.location="xltritem.php?action=new&xid='.$xid.'";</script>';
		...
		if($count>0)
			...
			$sql = "INSERT INTO stock_register (stock_id,entry_mode,entry_id,entry_date,seq_no,location_id,item_id,unit_id,item_qnty,item_rate) VALUES(".$sid.",'X+',".$xid.",'".$dateXLT."',".$sno.",".$row1['location_id'].",".$_POST['item'].",".$unitid.",".$itemQnty.",0)";
			$res = mysql_query($sql) or die(mysql_error());
			//insert into logbook
			$sql = mysql_query("SELECT Max(rec_id) as maxid FROM logbook");
			$row = mysql_fetch_assoc($sql);
			$recordid = ($row["maxid"]==null ? 1 : $row["maxid"] + 1);
			$sql = "INSERT INTO logbook (rec_id,voucher_id,voucher_date,voucher_type,entry_date,particulars,item_name,unit,item_qnty,location,action,user) VALUES(".$recordid.",'".$voucherid."','".$dateXLT."','XLT.Rcvd.','".date("Y-m-d")."','".$particulars."','".$itemname."','".$unitname."',".$_POST['itemQnty'].",'".$_SESSION['stores_lname']."','New','".$_SESSION['stores_uname']."')";
			$res = mysql_query($sql) or die(mysql_error());
			//end of inserting record into logbook
			echo '<script language="javascript">window.location="xltritem.php?action=new&xid='.$xid.'";</script>';
		}
...
<tr>
	...
	<tr>
		...
		<tr>
			...
			<td class="th"><strong>External Location Transfer - [ Receive Main ]</strong></td>
		...
		<tr class="Controls">
			...
			<td><input name="despatchFrom" id="despatchFrom" maxlength="50" size="45" readonly="true" value="<?php echo $tfr_location;?>" style="background-color:#E7F0F8; color:#0000FF"></td>
			
			<td class="th">Receive At:</td>
			<td><input name="receiveAt" id="receiveAt" maxlength="50" size="45" readonly="true" value="<?php echo $location_name;?>" style="background-color:#E7F0F8; color:#0000FF"><input type="hidden" name="location" id="location" value="<?php echo $location_id;?>" /></td>
		</tr>
		
		<tr class="Controls">
			<td class="th" nowrap>Received By:</td>
			<td><input name="receiveBy" id="receiveBy" maxlength="50" size="45" readonly="true" value="<?php echo $staff_name;?>" style="background-color:#E7F0F8; color:#0000FF"></td>
...
<tr>
	<td valign="top" colspan="3">
	<form name="xltritem"  method="post" onsubmit="return validate_xlt_item()">
	...
	<tr>
		...
		<?php } elseif(isset($_REQUEST["action"]) && $_REQUEST["action"]=="edit"){?>
			<input type="image" name="submit" src="images/update.gif" width="82" height="22" alt="update"><input type="hidden" name="submit" value="update"/>&nbsp;&nbsp;<a href="javascript:window.location='xltritem.php?action=new&xid=<?php echo $xid;?>'" ><img src="images/reset.gif" width="72" height="22" style="display:inline; cursor:hand;" border="0" /></a>
		<?php } elseif(isset($_REQUEST["action"]) && $_REQUEST["action"]=="delete"){?>
			<input type="image" name="submit" src="images/delete.gif" width="72" height="22" alt="delete"><input type="hidden" name="submit" value="delete"/>&nbsp;&nbsp;<a href="javascript:window.location='xltritem.php?action=new&xid=<?php echo $xid;?>'" ><img src="images/reset.gif" width="72" height="22" style="display:inline;cursor:hand;" border="0" /></a>
		<?php }?>
&nbsp;&nbsp;<a href="javascript:window.location='xltreceive1.php?xid=<?php echo $xid;?>'" ><img src="images/back.gif" width="72" height="22" style="display:inline;cursor:hand;" border="0" /></a>
			</td>
		</tr>
		</table>
		</td>
...
<tr>
	...
	<tr>
		...
		<tr>
			<td class="HeaderLeft"><img src="images/spacer.gif" border="0" alt=""></td>
			<td class="th"><strong>External Location Transfer - [ Receive Item List ]</strong></td>
			<td class="HeaderRight"><img src="images/spacer.gif" border="0" alt=""></td>
		</tr>
		</table>
		...
		while($row_xlt=mysql_fetch_array($sql_xlt)){
			$i++;
			echo '<tr class="Row">';
			$delete_ref = "xltritem.php?action=delete&xid=".$xid."&rid=".$row_xlt['rec_id'];
			$edit_ref = "xltritem.php?action=edit&xid=".$xid."&rid=".$row_xlt['rec_id'];
