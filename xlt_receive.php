...
<body>
            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    <button class="btn btn-default pull pull-right " data-toggle="modal" data-target="#material_modal">
                        <span class="glyphicon glyphicon-plus-sign"></span> XLT Receive</button>
                </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" id="dispatch_from" class="form-control">

                                <div id="dispatch_from_err" style="color:red; display:none;"></div>
                            </div>
                            <div class="col-md-3">
                                <label for="receive_at">Received At:</label>
                                <select id="receive_at" class="form-control">
                                ...
                                <div id="receive_at_err" style="color:red; display:none;"></div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <div id="received_by_err" style="color:red; display:none;"></div>
                            </div>
                            <div class="col-md-4">
                                <label for="received_mode">Received Mode:</label>
                                <select id="received_mode" class="form-control">
                                    <option value="">Select</option>
                                    <option value="1">Hand Delivery</option>
                                    <option value="2">By Vehicle</option>
                                </select>
                                <div id="received_mode_err" style="color:red; display:none;"></div>
                            </div>
    ...
    //======================================
    //-===----====--------=====---------====------------------------
    $(document).on('change', '#receive_at', function() {
    ...
    //==================================================//
    $(document).on('change', '#received_mode', function() {
        var r_mode = $(this).val();
        if (r_mode == 1) {
    ...
    $(document).on("click", "#save", function() {
        ...
        var dispatch_from = $('#dispatch_from').val();
        var receive_at = $('#receive_at').val();
        var received_by = $('#received_by').val();
        var received_mode = $('#received_mode').val();
        ...
        if (xlt_id) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#dispatch_from').val(response.tfr_location);
                    $('#receive_at').prepend("<option value='" + response.location_id + "' selected>" +
                        response.receive_at + "</option>");
                    $('#received_by').prepend("<option value='" + response.tfr_by + "' selected>" +
                        response.staff_name + "</option>");
                    if (response.despatch_mode == 1) {
                        $('#received_mode').prepend("<option value='" + 1 + "' selected>" +
                            'Hand Delivery' + "</option>");
                        $('#vehicle_div').css('display', 'none');
                    } else {
                        $('#received_mode').prepend("<option value='" + 2 + "' selected>" +
                            'By Vehicle' + "</option>");
                        $('#vehicle_div').css('display', 'block');
                    }
