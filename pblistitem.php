...
<head>
    ...
    function validate_dateselection() {
        if (checkdate(document.pblist.dateFrom)) {
            if (checkdate(document.pblist.dateTo)) {
                var no_of_days1 = getDaysbetween2Dates(document.pblist.dateFrom, document.pblist.dateTo);
                if (no_of_days1 < 0) {
                    alert("* Sorry! date range wrongly selected. Please correct and submit again.\n");
                    return false;
                } else {
                    var no_of_days2 = getDaysbetween2Dates(document.pblist.startYear, document.pblist.dateFrom);
                    if (no_of_days2 < 0) {
                        alert("* Report From date wrongly selected. Please correct and submit again.\n");
                        return false;
                    } else {
                        var no_of_days3 = getDaysbetween2Dates(document.pblist.dateTo, document.pblist.endYear);
                        if (no_of_days3 < 0) {
    ...
    function paging_pblist() {
        window.location = "pblistitem.php?pid=" + document.getElementById("product").value + "&pg=" + document
...
<body>
    ...
    <form name="pblist" id="pblist" method="post" action="pblistitem.php" onsubmit="return validate_dateselection()">
        <table align="center" border="0" cellpadding="2" cellspacing="1" width="1075px">
            <tbody>
                <tr align="center"
                    ...
                    <td>Item wise Purchase Bill List</td>
                </tr>
                <tr align="center"
                            ...
                            <table align="center" border="1" bordercolorlight="#7ECD7A" cellpadding="2" cellspacing="0"
                                ...
                                <tbody>
                                    <tr bgcolor="#E6E1B0" align="center"
                                        ...
                                        <td width="15%" valign="middle"
                                            style="border-top:none; border-left:none; border-bottom:none;">Bill to
                                            Company</td>
                                        <td width="15%" valign="middle"
                                            style="border-top:none; border-left:none; border-bottom:none;" colspan="2">
                                            Billing Qnty.</td>
                                        <td width="10%" valign="middle"
                                            style="border-top:none; border-left:none; border-bottom:none;">Item Amount
                                        </td>
                                        <td width="10%" valign="middle"
                                            style="border-top:none; border-left:none; border-bottom:none;">Against MR
                                            No.</td>
                                        <td width="10%" valign="middle"
	...
	$sql = "SELECT tblbill_item.*, tblbill.*, po_no, receipt_no, receipt_prefix, party_name, company_name, item_name, unit_name,ic.category FROM tblbill_item INNER JOIN tblbill ON tblbill_item.bill_id = tblbill.bill_id INNER JOIN tblpo ON tblbill_item.po_id = tblpo.po_id INNER JOIN tblreceipt1 ON tblbill_item.receipt_id = tblreceipt1.receipt_id INNER JOIN party ON tblbill.party_id = party.party_id INNER JOIN company ON tblbill.company_id = company.company_id INNER JOIN item ON tblbill_item.item_id = item.item_id INNER JOIN item_category ic ON ic.category_id = tblbill_item.item_category INNER JOIN unit ON tblbill_item.unit_id = unit.unit_id WHERE bill_return=0 AND (bill_date BETWEEN '".date("Y-m-d",$sm)."' AND '".date("Y-m-d",$em)."')";
	if($pid!=0){
		$sql .= " AND tblbill_item.item_id=".$pid;
	}
	$sql .= " ORDER BY item_name, bill_date, bill_no LIMIT ".$start.",".$end;
	...
	while($row=mysql_fetch_array($res)){
		...
		$itemvalue = number_format($row['bill_qnty'] * $row['rate'],2,'.','');
		...
		if($row['item_id']!=$itemNo){
			...
			echo '<td style="border-left:none; border-bottom:none; border-right:none; font-weight:bold" colspan="9">'.$row['item_name'].' ~~'.$row['category'].'</td>';
			echo '</tr>';
		}
		...
		echo '<td style="border-left:none; border-bottom:none; border-right:none" width="10%" align="right">'.($row['bill_qnty']==0?"&nbsp;":$row['bill_qnty']).'</td>';
		echo '<td style="border-left:none; border-bottom:none" width="5%" align="left">&nbsp;'.($row['bill_qnty']==0?"&nbsp;":$row['unit_name']).'</td>';
		...
		echo '</tr>';
                ...
                <tr>
                    <td align="center">
                        <?php 
	$sql = "SELECT tblbill_item.*, tblbill.* FROM tblbill_item INNER JOIN tblbill ON tblbill_item.bill_id = tblbill.bill_id WHERE bill_return=0 AND (bill_date BETWEEN '".date("Y-m-d",$sm)."' AND '".date("Y-m-d",$em)."')";
	if($pid!=0){
		$sql .= " AND tblbill_item.item_id=".$pid;
	}
