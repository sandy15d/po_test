...
if(isset($_REQUEST['mid'])){
	...
	if($_REQUEST["action"]=="edit" || $_REQUEST["action"]=="delete"){
		$sql = mysql_query("SELECT * FROM tblilt1 WHERE ilt_id=".$mid) or die(mysql_error());
		$row = mysql_fetch_assoc($sql);
		$location_id = $row['despatch_from'];
		$ilt_no = $row['ilt_no'];
		...
		$receive_at = $row["receive_at"];
		$despatch_by = $row["despatch_by"];
		...
		$row = mysql_fetch_assoc($sql);
		$dateILT = $row['ilt_date'];
		$particulars = "From ".$row['sourceLocation']." To ".$row['destinationLocation'];
		$res = mysql_query("UPDATE tblilt1 SET despatch_status='U' WHERE ilt_id=".$mid) or die(mysql_error());
		//insert into logbook
		...
		$sql = "INSERT INTO logbook (rec_id,voucher_id,voucher_date,voucher_type,entry_date,particulars,location,action,user) VALUES(".$recordid.",'".$voucherid."','".$dateILT."','ILT.Dspch.','".date("Y-m-d")."','".$particulars."','".$_SESSION['stores_lname']."','Recall','".$_SESSION['stores_uname']."')";
...
if(isset($_POST['submit'])){
	$dateILT=substr($_POST['iltDate'],6,4)."-".substr($_POST['iltDate'],3,2)."-".substr($_POST['iltDate'],0,2);
	...
	$sourcelocation = $row_loc['location_name'];
	/*-----------------------------*/
	$sql = mysql_query("SELECT location_name FROM location WHERE location_id=".$_POST['destination']) or die(mysql_error());
	$row = mysql_fetch_assoc($sql);
	$targetlocation = $row['location_name'];
	$particulars = "From ".$sourcelocation." To ".$targetlocation;
	/*-----------------------------*/
	if($_POST['submit']=="update"){
		if($_POST['location']!=$location_id){
			$sql = mysql_query("SELECT Max(ilt_no) as maxno FROM tblilt1 WHERE despatch_from=".$_POST['location']." AND (ilt_date BETWEEN '".date("Y-m-d",strtotime($_SESSION['stores_syr']))."' AND '".date("Y-m-d",strtotime($_SESSION['stores_eyr']))."')");
			$row = mysql_fetch_assoc($sql);
			$ino = $row["maxno"] + 1;
		} else {
			$ino = $ilt_no;
		}
		$sql = "UPDATE tblilt1 SET ilt_date='".$dateILT."',ilt_no=".$ino.",";
		if($row_loc['location_prefix']==null){$sql .= "ilt_prefix=null, ";} else {$sql .= "ilt_prefix='".$row_loc['location_prefix']."', ";}
		$sql .= "despatch_from=".$_POST['location'].",receive_at=".$_POST['destination'].",despatch_by=".$_POST['staffName'].",despatch_mode=".$_POST['despatchMode'].",vehicle_num='".$_POST['vehicleNumber']."' WHERE ilt_id=".$mid;
		$res = mysql_query($sql) or die(mysql_error());
		$res = mysql_query("UPDATE stock_register SET entry_date='".$dateILT."',location_id=".$_POST['location']." WHERE entry_mode='T-' AND entry_id=".$mid) or die(mysql_error());
		//insert into logbook
		$sql = mysql_query("SELECT Max(rec_id) as maxid FROM logbook");
		$row = mysql_fetch_assoc($sql);
		$recordid = ($row["maxid"]==null ? 1 : $row["maxid"] + 1);
		$voucherid = ($mid>999 ? $mid : ($mid>99 && $mid<1000 ? "0".$mid : ($mid>9 && $mid<100 ? "00".$mid : "000".$mid)));
		$sql = "INSERT INTO logbook (rec_id,voucher_id,voucher_date,voucher_type,entry_date,particulars,location,action,user) VALUES(".$recordid.",'".$voucherid."','".$dateILT."','ILT.Dspch.','".date("Y-m-d")."','".$particulars."','".$_SESSION['stores_lname']."','Change','".$_SESSION['stores_uname']."')";
		$res = mysql_query($sql) or die(mysql_error());
		//end of inserting record into logbook
		echo '<script language="javascript">window.location="iltditem.php?action=new&mid='.$mid.'";</script>';
	} elseif($_POST['submit']=="delete"){
		$res = mysql_query("DELETE FROM tblilt1 WHERE ilt_id=".$mid) or die(mysql_error());
		$res = mysql_query("DELETE FROM tblilt2 WHERE ilt_id=".$mid) or die(mysql_error());
		$res = mysql_query("DELETE FROM stock_register WHERE (entry_mode='T-' OR entry_mode='T+') AND entry_id=".$mid) or die(mysql_error());
		//insert into logbook
		$sql = mysql_query("SELECT Max(rec_id) as maxid FROM logbook");
		$row = mysql_fetch_assoc($sql);
		$recordid = ($row["maxid"]==null ? 1 : $row["maxid"] + 1);
		$voucherid = ($mid>999 ? $mid : ($mid>99 && $mid<1000 ? "0".$mid : ($mid>9 && $mid<100 ? "00".$mid : "000".$mid)));
		$sql = "INSERT INTO logbook (rec_id,voucher_id,voucher_date,voucher_type,entry_date,particulars,location,action,user) VALUES(".$recordid.",'".$voucherid."','".$dateILT."','ILT.Dspch.','".date("Y-m-d")."','".$particulars."','".$_SESSION['stores_lname']."','Delete','".$_SESSION['stores_uname']."')";
		$res = mysql_query($sql) or die(mysql_error());
		//end of inserting record into logbook
		echo '<script language="javascript">window.location="iltdespatch.php?action=new";</script>';
	} elseif($_POST['submit']=="new"){
		$sql = mysql_query("SELECT Max(ilt_id) as maxid FROM tblilt1");
		$row = mysql_fetch_assoc($sql);
		$mid = ($row["maxid"]==null ? 1 : $row["maxid"] + 1);
		$sql = mysql_query("SELECT Max(ilt_no) as maxno FROM tblilt1 WHERE despatch_from=".$_POST['location']." AND (ilt_date BETWEEN '".date("Y-m-d",strtotime($_SESSION['stores_syr']))."' AND '".date("Y-m-d",strtotime($_SESSION['stores_eyr']))."')");
		$row = mysql_fetch_assoc($sql);
		$ino = ($row["maxno"]==null ? 1 : $row["maxno"] + 1);
		$sql = "INSERT INTO tblilt1 (ilt_id,ilt_date,ilt_no,ilt_prefix,receive_date,despatch_from,receive_at,despatch_by,despatch_mode,vehicle_num) VALUES(".$mid.",'".$dateILT."',".$ino.",";
		if($row_loc['location_prefix']==null){$sql .= "null,";} else {$sql .= "'".$row_loc['location_prefix']."',";}
		$sql .= "'".$dateILT."',".$_POST['location'].",".$_POST['destination'].",".$_POST['staffName'].",".$_POST['despatchMode'].",'".$_POST['vehicleNumber']."')";
		$res = mysql_query($sql) or die(mysql_error());
//		header('location: iltditem.php?action=new&mid='.$mid);
		echo '<script language="javascript">function show_message_ilt_number(value1,value2){
			alert("ILT No. = "+value2);
			window.location="iltditem.php?action=new&mid="+value1;}
			show_message_ilt_number('.$mid.','.$ino.');</script>';
	}
}
?>
<!DOCTYPE html
...
<head>
    ...
    function validate_iltdata() {
        var err = "";
        if (document.getElementById("iltDate").value != "") {
            if (!checkdate(document.iltdespatch.iltDate)) {
                return false;
            } else {
                var no_of_days1 = getDaysbetween2Dates(document.iltdespatch.iltDate, document.iltdespatch.endYear);
                if (no_of_days1 < 0) {
                    err += "* ILT date wrongly selected. Please correct and submit again.\n";
                } else {
                    var no_of_days2 = getDaysbetween2Dates(document.iltdespatch.startYear, document.iltdespatch
                        .iltDate);
                    if (no_of_days2 < 0) {
                        err += "* ILT date wrongly selected. Please correct and submit again.\n";
                    } else {
                        var no_of_days3 = getDaysbetween2Dates(document.iltdespatch.maxDate, document.iltdespatch
                            .iltDate);
                        if (no_of_days3 < 0) {
                            err += "* ILT date wrongly selected. Please correct and submit again.\n" +
        ...
        } else if (document.getElementById("iltDate").value == "")
            err = "* please input/select ILT date!\n";
        ...
        if (document.getElementById("destination").value == 0)
            err += "* please select destination, where material being received at!\n";
        if (document.getElementById("staffName").value == 0)
            err += "* please select leader, by whom the material being despatched!\n";
    ...
    function paging_iltd() {
        if (document.getElementById("xson").value == "new") {
            window.location = "iltdespatch.php?action=" + document.getElementById("xson").value + "&pg=" + document
            ...
            window.location = "iltdespatch.php?action=" + document.getElementById("xson").value + "&mid=" + document
    ...
    function firstpage_iltd() {
        document.getElementById("page").value = 1;
        paging_iltd();
    }

    function previouspage_iltd() {
...
<body>
        ...
        <table align="center" cellspacing="0" cellpadding="0" height="250px" width="875px" border="0">
            <tr>
                <td valign="top" colspan="3">
                    <form name="iltdespatch" method="post" onsubmit="return validate_iltdata()">
                                ...
                                <td valign="top">
                                    <table class="Header" width="100%" cellspacing="0" cellpadding="0" border="0">
                                        <tr>
                                            ...
                                            <td class="th"><strong>Inter Location Transfer - [ Despatch Main ]</strong>
                                            </td>
                                    ...
                                    <table class="Record" width="100%" cellspacing="0" cellpadding="0">
                                        <tr class="Controls">
                                            ...
                                            <td class="th" nowrap>ILT Date:<span style="color:#FF0000">*</span></td>
                                            <td><input name="iltDate" id="iltDate" maxlength="10" size="10"
                                                    value="<?php echo $ilt_date;?>">&nbsp;<script language="JavaScript">
                                                new tcal({
                                                    'formname': 'iltdespatch',
                                                    'controlname': 'iltDate'
                                                });
                                                </script>
                                            </td>
                                        ...
                                        <tr class="Controls">
                                            <td class="th">Despatch From:<span style="color:#FF0000">*</span></td><?php 
			if($_SESSION['stores_utype']=="A" || $_SESSION['stores_utype']=="S"){
				...
				echo '<td><input name="locationName" id="locationName" maxlength="50" size="45" readonly="true" value="'.$_SESSION['stores_lname'].'" style="background-color:#E7F0F8; color:#0000FF">&nbsp;<input type="hidden" name="location" id="location" value="'.$_SESSION['stores_locid'].'" /></td>';
                                            ...
                                            <td><select name="destination" id="destination" style="width:300px">
                                                    <option value="0">-- Select --</option><?php 
			$sql_dest=mysql_query("SELECT * FROM location ORDER BY location_name");
			while($row_dest=mysql_fetch_array($sql_dest)){
				if($row_dest["location_id"]==$receive_at)
					echo '<option selected value="'.$row_dest["location_id"].'">'.$row_dest["location_name"].'</option>';
				else
					echo '<option value="'.$row_dest["location_id"].'">'.$row_dest["location_name"].'</option>';
			}?>
                                                </select></td>
                                        </tr>
			...
			while($row_staff=mysql_fetch_array($sql_staff)){
				if($row_staff["staff_id"]==$despatch_by)
			...
			if($row_user['ilt1']==1){?>
                                                ...
                                                <?php }?>
                                                &nbsp;&nbsp;<a href="javascript:document.iltdespatch.reset()"><img
                                                ...
                                                <input type="image" name="submit" src="images/update.gif" width="82"
                                                    ...
                                                    href="javascript:window.location='iltditem.php?action=new&mid=<?php echo $mid;?>'"><img
                                                        ...
                                                        border="0" /></a>&nbsp;&nbsp;<a
                                                    href="javascript:window.location='iltdespatch.php?action=new'"><img
                                                        src="images/reset.gif" width="72" height="22"
                                                        style="display:inline; cursor:hand;" border="0" /></a>
                            ...
                            <tr>
                                <td valign="top">
                                    <table class="Header" cellspacing="0" cellpadding="0" border="0" width="100%">
                                        <tr>
                                            <td class="HeaderLeft"><img src="images/spacer.gif" border="0" alt=""></td>
                                            <td class="th"><strong>Inter Location Transfer - [ Despatch List ]</strong>
                                            </td>
                                            <td class="HeaderRight"><img src="images/spacer.gif" border="0" alt=""></td>
                                        </tr>
                                    </table>
	...
	implementation of the dynamically generated pages. -->
                                    ...
                                    <table class="Grid" width="100%" cellspacing="0" cellpadding="0">
                                        <tr class="Caption">
                                            <th colspan="8">List Range:&nbsp;&nbsp;<select name="rangeFor" id="rangeFor"
                                                    style="width:110px" onchange="listrange(this.value)">
                                                    <?php 
		...
		$i = $start;
		if($_SESSION['stores_utype']=="A" || $_SESSION['stores_utype']=="S")
			$sql_ilt = mysql_query("SELECT tblilt1.*, source.location_name AS sourceLocation, destination.location_name AS destinationLocation, staff_name FROM tblilt1 INNER JOIN location AS source ON tblilt1.despatch_from = source.location_id INNER JOIN location AS destination ON tblilt1.receive_at = destination.location_id INNER JOIN staff ON tblilt1.despatch_by = staff.staff_id WHERE despatch_status='".$rangeFor."' AND ilt_date BETWEEN '".$fromDate."' AND '".$toDate."' ORDER BY sourceLocation,ilt_date,ilt_id LIMIT ".$start.",".$end) or die(mysql_error());
		elseif($_SESSION['stores_utype']=="U")
			$sql_ilt = mysql_query("SELECT tblilt1.*, source.location_name AS sourceLocation, destination.location_name AS destinationLocation, staff_name FROM tblilt1 INNER JOIN location AS source ON tblilt1.despatch_from = source.location_id INNER JOIN location AS destination ON tblilt1.receive_at = destination.location_id INNER JOIN staff ON tblilt1.despatch_by = staff.staff_id WHERE despatch_from=".$_SESSION['stores_locid']." AND despatch_status='".$rangeFor."' AND ilt_date BETWEEN '".$fromDate."' AND '".$toDate."' ORDER BY sourceLocation,ilt_date,ilt_id LIMIT ".$start.",".$end) or die(mysql_error());
		...
		while($row_ilt=mysql_fetch_array($sql_ilt)){
			...
			while($row_item=mysql_fetch_array($sql_item)){
				$stext .='<tr>';
				$j++;
				$stext .='<td>'.$j.'</td><td>'.$row_item['item_name'].'</td><td>'.$row_item['despatch_qnty'].' '.$row_item['unit_name'].'</td>';
				$stext .='</tr>';
			}
			$stext .='</table>';
			...
			$i++;
			echo '<tr class="Row">';
			if($row_ilt['despatch_status']=='S'){
				$edit_ref = "iltdespatch.php?action=recall&mid=".$row_ilt['ilt_id'];
			} elseif($row_ilt['despatch_status']=='U'){
				$edit_ref = "iltdespatch.php?action=edit&mid=".$row_ilt['ilt_id'];
			}
			$delete_ref = "iltdespatch.php?action=delete&mid=".$row_ilt['ilt_id'];
			$x = "window.open('printofpage.php?typ=ilt&vid=".$row_ilt['ilt_id']."', 'voucherPrint', 'width=300,height=200, resizable=no, scrollbars=no, toolbar=no, location=no, directories=no, status=no, menubar=no, copyhistory=no')";
			...
			echo '<td align="center">'.$i.'.</td><td onmouseover="myShow(\'<b><u>This is ILT number '.$ilt_number.'</u></b><br/>'.$stext.'\', this)" onmouseout="myHint.hide()">'.$ilt_number.'</td><td align="center">'.date("d-m-Y",strtotime($row_ilt['ilt_date'])).'</td><td>'.$row_ilt['sourceLocation'].'</td><td>'.$row_ilt['destinationLocation'].'</td><td>'.$row_ilt['staff_name'].'</td>';
			echo '<td align="center">&nbsp;';
			if($row_user['ilt2']==1){
				if($row_ilt['despatch_status']=='S'){
					echo '<a href="'.$edit_ref.'"><img src="images/undo.gif" title="Recall" style="display:inline;cursor:hand;" border="0" /></a>';
				} elseif($row_ilt['despatch_status']=='U'){
			...
			echo '&nbsp;</td></tr>';
                                        ...
                                        <tr class="Footer">
                                            <td colspan="8" align="center">
                                                <?php 
			if($_SESSION['stores_utype']=="A" || $_SESSION['stores_utype']=="S")
				$sql_total = mysql_query("SELECT * FROM tblilt1 WHERE despatch_status='".$rangeFor."' AND ilt_date BETWEEN '".$fromDate."' AND '".$toDate."'") or die(mysql_error());
			elseif($_SESSION['stores_utype']=="U")
				$sql_total = mysql_query("SELECT * FROM tblilt1 WHERE despatch_from=".$_SESSION['stores_locid']." AND despatch_status='".$rangeFor."' AND ilt_date BETWEEN '".$fromDate."' AND '".$toDate."'") or die(mysql_error());
			$tot_row=mysql_num_rows($sql_total);
			$total_page=0;
			echo 'Total <span style="color:red">'.$tot_row.'</span> records&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
			echo '<input type="button" name="show" id="show" value="Show:" onclick="paging_iltd()" />&nbsp;&nbsp;<input name="displayTotalRows" id="displayTotalRows" value="'.$end.'" maxlength="2" size="2"/> rows&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="hidden" name="xson" id="xson" value="'.$_REQUEST["action"].'" /><input type="hidden" name="miid" id="miid" value="'.$mid.'" />';
			if($tot_row>$end){
				echo "Page number: ";
				$total_page=ceil($tot_row/$end);
				echo '<select name="page" id="page" onchange="paging_iltd()" style="vertical-align:middle">';
			...
			if($total_page>1 && $pg>1)
				echo '<input type="button" name="fistPage" id="firstPage" value=" << " onclick="firstpage_iltd()" />&nbsp;&nbsp;<input type="button" name="prevPage" id="prevPage" value=" < " onclick="previouspage_iltd()" />&nbsp;&nbsp;';
			if($total_page>1 && $pg<$total_page)
				echo '<input type="button" name="nextPage" id="nextPage" value=" > " onclick="nextpage_iltd()" />&nbsp;&nbsp;<input type="button" name="lastPage" id="lastPage" value=" >> " onclick="lastpage_iltd()" />';
			?>
                                            </td>
                                        </tr>
                                    </table>
